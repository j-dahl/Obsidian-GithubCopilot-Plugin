...
export class ChatViewModel {
  ...
  constructor(context: ChatPluginContext, events: ChatViewModelEvents, app?: App) {
    ...
    const saved = this.validSavedConversations();
    this.conversations = saved.length > 0 ? saved : [conversation];
    this.currentConversationId =
      this.context.settings.currentChatConversationId &&
      this.conversations.some((item) => item.id === this.context.settings.currentChatConversationId)
        ? this.context.settings.currentChatConversationId
        : (this.conversations[0]?.id ?? conversation.id);
  ...
  renameConversation(id: string, title: string): void {
    ...
    if (!conversation || trimmed.length === 0) return;
  ...
  deleteConversation(id: string): void {
    ...
    if (this.conversations.length === 0) {
      ...
      const next = this.conversations[Math.min(index, this.conversations.length - 1)];
      this.currentConversationId = next?.id ?? this.conversations[0]?.id ?? "";
    }
    this.emitChange();
  }
  ...
  async sendUserMessage(text: string, attachedFiles: string[]): Promise<void> {
    ...
    this.addMessage({
      id: this.id(),
      role: "user",
      content: [trimmed, contextText].filter((part) => part.length > 0).join("\n\n"),
      timestamp: Date.now(),
    });
  ...
  async regenerate(): Promise<void> {
    ...
    const lastAssistant = conversation.messages.findLastIndex(
  ...
  private async runAgentLoop(runId: number, signal: AbortSignal, depth: number): Promise<void> {
    if (depth >= 10) {
      this.addMessage({
        id: this.id(),
        role: "tool",
        content: "Recursion cap hit after 10 tool iterations.",
        timestamp: Date.now(),
      });
      this.runState = "error";
      this.emitChange();
    ...
    try {
      for await (const chunk of this.context.provider.stream({
  ...
  private async resolveToolCalls(toolCalls: ToolCall[], signal: AbortSignal): Promise<void> {
    for (const toolCall of toolCalls) {
      ...
      if (decision.action === "deny") {
        ...
        await this.context.auditLogger.logToolCall({
          toolCall: denied,
          decision: "deny",
          timestamp: Date.now(),
        });
      ...
      this.emitChange();
      try {
        const result = await this.context.dispatcher.callTool(
          toolCall.serverName,
          toolCall.name,
          toolCall.arguments,
          signal
  ...
  private disabledToolsFor(serverName: string): string[] {
    ...
    if (Array.isArray(servers)) {
      return (
        servers.find((server) => server.id === serverName || server.name === serverName)
          ?.disabledTools ?? []
      );
    }
  ...
  private async buildAttachedFileContext(
    ...
    for (const path of attachedFiles) {
      ...
      const content = this.events.readVaultFile
        ? await this.events.readVaultFile(validateVaultRelativePath(path), signal)
        : "";
      blocks.push(
  ...
  private addToolMessage(toolCall: ToolCall): void {
    ...
    const llmContent =
      toolCall.result === undefined ? content : wrapToolResult(toolCall, toolCall.result);
    this.addMessage({
      id: this.id(),
      role: "tool",
      content,
  ...
  private addMessage(message: ChatMessage): void {
    ...
    this.currentConversation.updatedAt = Date.now();
    if (message.role === "user" && isDefaultConversationTitle(this.currentConversation.title)) {
      this.currentConversation.title = titleFromFirstMessage(message.content);
    }
    this.emitChange();
  }
  ...
  private createConversation(): Conversation {
    ...
    return {
      id: this.id(),
      title: `New chat (${formatConversationDate(now)})`,
      messages: [],
      createdAt: now,
      updatedAt: now,
    };
  ...
  private validSavedConversations(): Conversation[] {
    const saved = this.context.settings.chatConversations;
    if (!Array.isArray(saved)) return [];
    return saved.filter((conversation): conversation is Conversation => {
      return (
  ...
  private persistConversationState(): void {
    this.context.settings.chatConversations = this.conversations.map((conversation) => ({
      ...conversation,
      messages: [...conversation.messages],
    }));
    this.context.settings.currentChatConversationId = this.currentConversationId;
    void (this.context.saveChatState ?? this.context.saveSettings)?.();
  }
...
function isDefaultConversationTitle(title: string): boolean {
  return title === "New chat" || /^New chat \(\d{4}-\d{2}-\d{2} \d{2}:\d{2}\)$/.test(title);
}
...
function createNativeToolsFacade(): OpenAITool[] {
  const tools: Array<{
  ...
  }> = [
    {
      ...
      description: "Read the active file",
    ...
    },
    {
      ...
      description: "Get active selection",
      annotations: { readOnlyHint: true },
    },
