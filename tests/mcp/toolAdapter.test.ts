...
describe('toolAdapter', () => {
  test('passes MCP input schema directly to OpenAI tools', (): void => {
    const schema = { type: 'object' as const, properties: { query: { type: 'string' } }, required: ['query'] };
  ...
  test('uses annotation title and no-description fallbacks', (): void => {
    const titled: Tool = { name: 'a', annotations: { title: 'Title' }, inputSchema: { type: 'object' } };
