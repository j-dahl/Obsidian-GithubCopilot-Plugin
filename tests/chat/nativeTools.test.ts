  ...
  const app = {
    ...
    vault: {
      ...
      getAbstractFileByPath: (path: string) =>
        byPath.get(path) ?? created.find((file) => file.path === path) ?? null,
...
describe("nativeTools", () => {
  test("all native tool object schemas are explicit for strict providers", () => {
    const app = appWithFiles([]);
    for (const registration of createNativeTools(app)) {
      expect(registration.tool.inputSchema.type).toBe("object");
      expect(registration.tool.inputSchema).toHaveProperty("properties");
      expect(registration.tool.inputSchema).toHaveProperty("required");
      expect(registration.tool.inputSchema).toHaveProperty("additionalProperties", false);
    }
  });

  test("read_active_file reads workspace active file", async () => {
    const file = fakeFile("active.md", "active content");
    await expect(callTool(appWithFiles([file], file), "read_active_file")).resolves.toEqual({
  ...
  test("read_vault_file reads by path", async () => {
    ...
    await expect(
      callTool(appWithFiles([file]), "read_vault_file", { path: "note.md" })
    ).resolves.toEqual({ content: [{ type: "text", text: "note content" }] });
  });

  test("get_active_selection returns editor selection", async () => {
    await expect(
      callTool(appWithFiles([], null, "selected"), "get_active_selection")
    ).resolves.toEqual({ content: [{ type: "text", text: "selected" }] });
  });
  ...
  test("search_vault returns matching file paths", async () => {
    ...
    await expect(
      callTool(appWithFiles([first, second]), "search_vault", { query: "beta", deep: true })
    ).resolves.toEqual({ content: [{ type: "text", text: "a.md\nalpha beta" }] });
  });
  ...
  test("list_vault_files filters folders non-recursively", async () => {
    ...
    await expect(
      callTool(appWithFiles([first, second]), "list_vault_files", {
        folder: "folder",
        recursive: false,
      })
    ).resolves.toEqual({ content: [{ type: "text", text: "folder/a.md" }] });
  });

  test("create_note creates a new note", async () => {
    await expect(
      callTool(appWithFiles([]), "create_note", { path: "new.md", content: "new" })
    ).resolves.toEqual({ content: [{ type: "text", text: "Created new.md" }] });
  });

  test("append_note appends content", async () => {
    const file = fakeFile("append.md", "one");
    await expect(
      callTool(appWithFiles([file]), "append_note", { path: "append.md", content: " two" })
    ).resolves.toEqual({ content: [{ type: "text", text: "Appended to append.md" }] });
  ...
  test("delete_note trashes a note", async () => {
    ...
    await expect(
      callTool(appWithFiles([file]), "delete_note", { path: "delete.md" })
    ).resolves.toEqual({ content: [{ type: "text", text: "Moved delete.md to trash" }] });
  });
});
