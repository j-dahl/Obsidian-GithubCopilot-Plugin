...
function context(
  ...
  return {
    ...
    auditLogger: audit,
    trustedContent: {
      wrapForLlm: (path, content) => `<trusted path="${path}">${content}</trusted>`,
    },
    settings: {},
    systemPrompt: "system",
  };
...
describe("ChatViewModel", () => {
  test("single text turn without tools", async () => {
    ...
    const vm = new ChatViewModel(
      context(provider, new Gate({ action: "auto-allow" }), new Dispatcher()),
      { onConsent: () => Promise.resolve("allow-once") }
    );
    await vm.sendUserMessage("Hi", []);
    expect(vm.currentConversation.messages.map((message) => message.role)).toEqual([
      "user",
      "assistant",
    ]);
  ...
  test("auto-titles the conversation from the first user message", async () => {
    const provider = new ScriptedProvider([[{ content: "hello" }]]);
    const settings: ChatPluginContext["settings"] = {};
    const ctx = context(provider, new Gate({ action: "auto-allow" }), new Dispatcher());
    ctx.settings = settings;
    ctx.saveSettings = jest.fn(async () => undefined);
    const vm = new ChatViewModel(ctx, { onConsent: () => Promise.resolve("allow-once") });

    await vm.sendUserMessage(
      "**Plan** the catalog refresh from Obsidian settings with extra words",
      []
    );

    expect(vm.currentConversation.title).toBe("Plan the catalog refresh from Obsidian…");
    expect(settings.chatConversations?.[0]?.title).toBe(vm.currentConversation.title);
  ...
  test("surfaces no_token provider errors for sign-in recovery", async () => {
    ...
    const vm = new ChatViewModel(
      context(new MissingTokenProvider(), new Gate({ action: "auto-allow" }), new Dispatcher()),
      { onConsent: () => Promise.resolve("allow-once") }
    );
    await vm.sendUserMessage("Hi", []);
  ...
  test("auto-allowed tool call dispatches and recurses", async () => {
    ...
    const vm = new ChatViewModel(
      context(provider, new Gate({ action: "auto-allow" }), dispatcher, audit),
      { onConsent: () => Promise.resolve("allow-once") }
    );
    await vm.sendUserMessage("Use tool", []);
    expect(dispatcher.calls).toHaveLength(1);
    expect(audit.entries).toEqual(["auto-allow"]);
    expect(vm.currentConversation.messages.some((message) => message.content === "tool ok")).toBe(
      true
    );
  });
  ...
  test("consent allow-once dispatches requested tool", async () => {
    const provider = new ScriptedProvider([
      [{ toolCalls: [toolCall("t2")] }],
      [{ content: "after consent" }],
    ]);
    const dispatcher = new Dispatcher();
    const decisions: ConsentDecision[] = [];
    const vm = new ChatViewModel(
      context(provider, new Gate({ action: "ask", reason: "policy" }), dispatcher),
      {
        onConsent: () => {
  ...
  test("tool results use base64 untrusted envelope for provider messages", async () => {
    ...
    class InspectingProvider implements ChatCompletionProvider {
      ...
      async *stream(request: {
        ...
        sawWrapped = request.messages.some(
          (message) =>
            message.role === "tool" &&
            message.content.includes("<untrusted-tool-result") &&
            message.content.includes("dG9vbCBvaw==")
        );
        yield { content: "done" };
      }
    }
    const vm = new ChatViewModel(
      context(new InspectingProvider(), new Gate({ action: "auto-allow" }), new Dispatcher()),
      { onConsent: () => Promise.resolve("allow-once") }
    );
    await vm.sendUserMessage("Use tool", []);
  ...
  test("denied policy creates synthetic tool message", async () => {
    ...
    const dispatcher = new Dispatcher();
    const vm = new ChatViewModel(
      context(provider, new Gate({ action: "deny", reason: "blocked" }), dispatcher),
      { onConsent: () => Promise.resolve("allow-once") }
    );
    await vm.sendUserMessage("Use tool", []);
    expect(dispatcher.calls).toHaveLength(0);
    expect(
      vm.currentConversation.messages.some((message) =>
        message.content.includes("Denied by policy: blocked")
      )
    ).toBe(true);
  });
  ...
  test("tool server error is rendered", async () => {
    const provider = new ScriptedProvider([
      [{ toolCalls: [toolCall("t4")] }],
      [{ content: "continued" }],
    ]);
    const vm = new ChatViewModel(
      context(
        provider,
        new Gate({ action: "auto-allow" }),
        new Dispatcher(new Error("server down"))
      ),
      { onConsent: () => Promise.resolve("allow-once") }
    );
    await vm.sendUserMessage("Use tool", []);
    expect(
      vm.currentConversation.messages.some((message) => message.content === "server down")
    ).toBe(true);
  });

  test("recursion cap stops infinite tool loops", async () => {
    const provider = new ScriptedProvider(
      Array.from({ length: 11 }, (_unused, index) => [{ toolCalls: [toolCall(`t${index}`)] }])
    );
    const vm = new ChatViewModel(
      context(provider, new Gate({ action: "auto-allow" }), new Dispatcher()),
      { onConsent: () => Promise.resolve("allow-once") }
    ...
    expect(
      vm.currentConversation.messages.some((message) =>
        message.content.includes("Recursion cap hit")
      )
    ).toBe(true);
  ...
  test("stop-generation aborts the stream", async () => {
    ...
    const vm = new ChatViewModel(
      context(new HangingProvider(), new Gate({ action: "auto-allow" }), new Dispatcher()),
      { onConsent: () => Promise.resolve("allow-once") }
    );
    const pending = vm.sendUserMessage("Stop", []);
    for (
      let attempt = 0;
      attempt < 10 && vm.currentConversation.messages[1]?.content !== "partial";
      attempt += 1
    ) {
